...
const A4 = React.forwardRef(({
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(entries => {
      for (let entry of entries) {
        if (entry.target === orcamentoRef.current && a46InnerRef.current) {
          // Adjust the height of A46Inner to match orcamentogeral
    ...
    // Observe the orcamentogeral element for height changes
  ...
  const extrairValor = (razao) => {
    // Extrai o valor numérico de razao, que está formatada como "R${valor} p/{unidade}"
    const valor = razao.match(/R\$(\d+(?:\.\d+)?)/); // Regex para capturar o valor numérico
    return valor ? parseFloat(valor[1]) : 0; // Converte a string capturada para float
  };
  // Calcula o subtotal somando os valores de todos os itens
  const calcularSubtotal = () => {
    return itens.reduce((subtotal, item) => {
      const valorItem = extrairValor(item.razao) * item.quantidade;
      return subtotal + valorItem;
    }, 0); // Inicia com subtotal 0
  };

  const subtotal = calcularSubtotal(); // Calcula o subtotal
  // Converte valorFrete e valorDeslocamento para float
